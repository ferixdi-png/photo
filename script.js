...
<section class="chapter chapter--morning theme-cream" data-theme="cream">
  <div class="chapter-heading"><span>01</span><h2>Утро,<br><em>без спешки</em></h2><p>Городской ритуал. Лён, холодный кофе и спокойная уверенность дневного света.</p></div>
  <div class="triptych">${frame(1,'frame--tall','01.01 — Городской свет')}${frame(2,'frame--lift','01.02 — Мимолётный взгляд')}${frame(3,'frame--low','01.03 — Ритуал')}</div>
</section>
...
<section class="chapter theme-sky" data-theme="sky">
  <div class="chapter-heading chapter-heading--right"><span>02</span><h2>Открытый<br><em>воздух</em></h2><p>Ветер, высота, лето и движение. Пространство раскрывается, тело отвечает.</p></div>
  <div class="spread spread--sky">${frame(19,'frame--hero','02.01 — Летняя тишина')}${frame(11,'frame--narrow','02.02 — Над городом')}</div>
  <div class="cinema-strip"><div class="cinema-strip__label">ДВИЖЕНИЕ · 02.03—02.04</div><figure>${img(17)}</figure><figure>${img(20)}</figure></div>
  <div class="triptych" style="margin-top:14vh">${frame(26,'frame--tall','02.05 — Свет корта')}${frame(27,'frame--lift','02.06 — Городской стиль')}${frame(28,'frame--low','02.07 — У воды')}</div>
</section>
<section class="chapter theme-concrete" data-theme="concrete">
  <div class="chapter-heading"><span>03</span><h2>Форма<br><em>и тень</em></h2><p>Архитектура становится вторым телом: строгим, тихим и точным.</p></div>
  <div class="architecture-grid">${frame(6,'frame--architect','03.01 — Структура')}${frame(5,'frame--dark','03.02 — Линия')}${frame(16,'frame--flash','03.03 — Двойное присутствие')}</div>
  <div class="triptych" style="margin-top:14vh">${frame(22,'frame--tall','03.04 — Скульптурная линия')}${frame(24,'frame--lift','03.05 — Спокойная точность')}${frame(23,'frame--low','03.06 — Нижняя точка')}</div>
</section>
...
<section class="chapter chapter--rooms theme-milk" data-theme="milk">
  <div class="chapter-heading"><span>04</span><h2>Личные<br><em>заметки</em></h2><p>Тихо, прямо и без защиты — домашняя последовательность в естественном свете.</p></div>
  <div class="rooms-grid">${frame(9,'room room--one','04.01')}${frame(12,'room room--two','04.02')}${frame(13,'room room--three','04.03')}<div class="rooms-grid__statement"><span>ТИХО / ПРЯМО / БЕЗ ЗАЩИТЫ</span><p>Визуальный язык становится личнее, но не теряет авторской точности.</p></div>${frame(14,'room room--four','04.04')}${frame(15,'room room--five','04.05')}${frame(18,'room room--six','04.06')}<figure class="frame room" style="grid-column:1/6;margin-top:14vh">${img(25)}<figcaption>04.07</figcaption></figure><figure class="frame room" style="grid-column:8/13;margin-top:24vh">${img(29)}<figcaption>04.08</figcaption></figure></div>
</section>
<section class="chapter chapter--night theme-night" data-theme="night">
  <div class="chapter-heading chapter-heading--light"><span>05</span><h2>После<br><em>полуночи</em></h2><p>Вспышка, кожа, цветы и личный свет. Последняя глава поворачивается внутрь.</p></div>
  <div class="night-sequence">${frame(7,'night-sequence__flowers','05.01 — Цветы после полуночи')}${frame(10,'night-sequence__leather','05.02 — Чёрная кожа')}${frame(21,'night-sequence__bed','05.03 — Личный свет')}</div>
</section>`;
...
setTimeout(()=>loader.classList.add('is-hidden'),5000);
...
function show(n){current=(n+TOTAL)%TOTAL;counter.textContent=`${pad(current+1)} / ${TOTAL}`;canvas.innerHTML=`<img src="${photoUrl(current+1)}" width="720" height="1280" alt="Кадр ${current+1}">`;if(!box.open)box.showModal()}
